...
RSpec.describe Sorn, type: :model do
  ...
  describe ".parse_xml" do
    ...
    it "parses as expected" do
      ...
      expect(sorn.summary).to start_with "GSA is publishing this system"
      ...
      expect(sorn.security).to eq "<p>Unclassified.</p>"
      expect(sorn.location).to eq "<p>National Computer Center in Research Triangle Park, North Carolina.</p>"
      expect(sorn.manager).to include "<p>The system manager is the Associate Chief Information Officer of Corporate IT Services in GSA-IT"
      expect(sorn.authority).to eq "<p>e-Government Act of 2002, see 44 U.S.C. 3602(f)(6); see also id § 3501, note.</p>"
      expect(sorn.purpose).to include "<p>The purpose of the e-Rulemaking Program Administrative System is to"
      expect(sorn.categories_of_individuals).to include "<p>Covered individuals are partner agency users who"
      expect(sorn.categories_of_record).to start_with "<p>GSA maintains partner agencies' users' names, government issued email addresses,"
      expect(sorn.source).to start_with "<p>The information in the system may be submitted"
      expect(sorn.routine_uses).to start_with "<p>In addition to those disclosures generally permitted under 5 U.S.C. 552a(b)"
      expect(sorn.storage).to start_with "<p>User credentials and associated documentation are stored on"
      expect(sorn.retrieval).to start_with "<p>The e-Rulemaking Program Administrative System retrieves"
      expect(sorn.retention).to start_with "<p>Records relating to user credentials"
      expect(sorn.safeguards).to start_with "<p>The e-Rulemaking Program Administrative System is in a facility"
      expect(sorn.access).to start_with "<p>Partner agency users can access and manage their user credentials through"
      expect(sorn.contesting).to start_with "<p>If partner agency users have questions"
      expect(sorn.notification).to start_with "<p>If partner agency users wish to receive notice about their account records,"
      expect(sorn.exemptions).to eq "<p>None.</p>"
